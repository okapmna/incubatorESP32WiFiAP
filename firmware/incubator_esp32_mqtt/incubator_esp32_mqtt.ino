...
#define RELAY_HUM_PIN 2
#define SDA_PIN 21
#define SCL_PIN 22

// MQTT Config
// Place your MQTT server details
...
unsigned long lastWifiCheck = 0; // Timer untuk cek wifi
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (!error) {
    ...
    Serial.println("Target diperbarui dan disimpan ke Preferences");
  }
}
...
void setup() {
  ...
  // Aagar tidak memblokir kode Pemanas tetap jalan saat config WiFi
  wm.setConfigPortalBlocking(false); 
  
  // Coba connect, jika gagal dia akan lanjut
  if(wm.autoConnect("ESP32_Incubator_AP")) {
      Serial.println("Terhubung ke WiFi (Boot)");
      lcd.setCursor(0,0); lcd.print("WiFi Connected");
  } else {
      Serial.println("WiFi Config Portal Aktif");
      lcd.setCursor(0,0); lcd.print("Connect AP: ESP32");
  }
  delay(1000); 
  lcd.clear();
...
void loop() {
  // Menjalankan proses WiFiManager di setiap loop
  wm.process();
  ...
  if (now - lastWifiCheck >= 1000) {
    ...
    if (WiFi.status() != WL_CONNECTED) {
      // Cek apakah portal konfigurasi (AP) sudah nyala?
      if (!wm.getConfigPortalActive()) {
        Serial.println("WiFi Putus! Memulai Config Portal...");
        // Nyalakan AP agar bisa disetting ulang lewat AP lewat  WEB
  ...
  if (WiFi.status() == WL_CONNECTED) {
    if (!client.connected()) {
      reconnect(); // Panggil fungsi reconnect non-blocking
    } else {
      client.loop();
    }
  }
  ...
  // Kode ini akan tetap jalan walaupun sedang setting WiFi
  if (now - lastSensorRead >= 2000) {
    ...
    if (!isnan(current_temp) && !isnan(current_hum)) {
      ...
      // Tampilan LCD
      // Jika disconect wifi, tampilkan info AP di baris 1 lcd
      if (wm.getConfigPortalActive()) {
        lcd.setCursor(0,0); lcd.print("SETUP WIFI MODE ");
      } else {
        lcd.setCursor(0,0);
